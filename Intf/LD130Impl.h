...
public:
    ...
    void getVersion(const char** verMajor, const char** verMinor,const char** verBuild) const
    {
		if (verMajor) { *verMajor = m_verMajor.c_str(); }
		if (verMinor) { *verMinor = m_verMinor.c_str(); }
		if (verBuild) { *verBuild = m_verBuild.c_str(); }
    }
...
private:
    ...
    */
	...
	{
	  public:
		TCritical (CRITICAL_SECTION& cs_)
			: cs(cs_)
		{

...
typedef void (*TLD130OnLogEvent)(const void* anObject, const char* pCmd, const char* pLogStr);
 ...
 *
 * anObject is the parameter that will be passed to the
 * onLogEvent callback
 *
...
extern "C"
#endif
LD130I_API BOOL LD130Init(HANDLE aHandle, int aComPort, int aSpeed = 115200, TLD130OnLogEvent onLogEvent = 0, const void* anObject=0);
...
/**
 * LD130isConnected() returns the connection status.
 *
 * @param aHandle is the handle returned LD130Init() function
 *
 * @return bool true if the the controller is connected
 *
 *
*/
#ifdef __cplusplus
...
/**
 * LD130SetBankSequence() sets the bank sequence
 *
 * @param aHandle is the handle returned LD130Init() function.
 * const char* aValue pointer to a bank sequence, where each
 * element is a character bank ID ['1','2','3','4']
 *
 * @return bool true if the command succedded, false in case of
 *         error
 *
...
/**
 * LD130GetActiveBank() returns the active bank ID
 *
 * @param aHandle is the handle returned LD130Init() function.
 *
 * @return Bank ID = 1,2,3,4 when BankId=0, then banks are not
 *  	   used
 *
 *
*/

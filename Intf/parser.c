 ...
 */
static char VarNames[VALUES_COUNT][STRING_NAME_LENGTH+1];	//49*16=784b
 ...
 */
#if defined(_MSC_VER) || defined(__BORLANDC__)
static unsigned char ValuesIdx[PATTERNS_COUNT][MAX_VALUES_IN_PATTERN];// 27*50=1350
#else
...
unsigned char ParseSentence(TCMDParser* pParser, char * pSentenceIn)
{
   ...
   char * pValue; //pointer to variable value
    ...
    // clear last CMD
    memset(pParser->CmdName, 0, sizeof(pParser->CmdName));
    // clear the buffer
    memset(pParser->VarValues, 0, sizeof(pParser->VarValues));
    ...
    for(i = 0; i < PATTERNS_COUNT; ++i)
    {
       ...
       for(j=0; j <= patternLength; j++)
       {
           ...
           if(CmdTemplates[i][j] == TokenSeparator  || (CmdTemplates[i][j] == 0 && CmdTemplates[i][j] == pSentenceIn[j]) )
           {
               ...
               varLength = j;
               if (varLength > sizeof(pParser->CmdName)-1)
                   varLength = sizeof(pParser->CmdName)-1;

               // copy the last command name
               strncpy(pParser->CmdName, pSentenceIn, varLength);
       ...
       if (valIdx != NOT_MATCH_INDEX)
           pValue = (char *) pParser->VarValues[ valIdx ];
       else
           pValue = 0;
...
const char * GetValueByNum(const TCMDParser* pParser, unsigned char anIndex)
{
    ...
    return (const char *) pParser->VarValues[anIndex];
}
//-----------------------------------------------------------------------------------------
...
const char * GetValueByName(const TCMDParser* pParser, const char * pName)
{
    ...
    for(i = 0; i < VALUES_COUNT; ++i)
    {
        ...
        if(strcmp(pName, name)==0)
        {
			...
			DbgOut(pName);
			DbgOut("=");
			DbgOut(pParser->VarValues[i]);
			DbgOut("} ");
#endif
            return (const char *) pParser->VarValues[i];
        }
    }

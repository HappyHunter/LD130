...
struct TSendLogger
{
	...
	~TSendLogger()
	{
		...
		char str[1024] = {0};
		snprintf (str, sizeof(str), "%02d:%02d:%02d %03d %s: %s %s (%d)", sysTime.wHour,sysTime.wMinute,sysTime.wSecond,sysTime.wMilliseconds, m_cmd, m_retCode.m_errorId, (char*)m_retCode.m_errorDescription, m_retCode.m_invalidParameterIndex);
		if (m_pLogEvent)
...
//-----------------------------------------------------------------------------------------
const char *  TSerialConMan::GetValueByName(const char * pName)
{
	return ::GetValueByName(pName);
}

//-----------------------------------------------------------------------------------------
const char *  TSerialConMan::GetCmdName()
{
	return ::GetCmdName();
}
